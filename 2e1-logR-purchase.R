...
library(caTools)
set.seed(2000)
split = sample.split(dataset$purchased, SplitRatio = 0.75) # use depended variable ir - purchase
...
summary(logitmodel1) # gender is not significant other variables are significant.

# modeling is done on the training set and test it on the test set
...
cm = table(test_set[,5], y_pred) #getting 5th column which is the purchase
cm
library(caret) # automatically gives the xonfusion matrix
